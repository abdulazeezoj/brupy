...
- **Strictly opinionated** — Brupy always uses this exact name and
  ...
  depend on.
- **A default, not a rule** — `schemas/` and `models/` are already
  packages, one file per resource, mirroring `routes/`/`tasks/`; and
  `scripts/` is a place brupy always provides for one-off/operational
  scripts, though nothing requires anything to live there.

Notice `models/` and the engine/session setup in `db.py` live outside
...
!!! warning "With `migrations=true`, the database is not created on startup"
    ...
    `models/`. Always review an autogenerated migration before
    applying it.
